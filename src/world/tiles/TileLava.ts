...
export default class TileLava extends WorldObject {
    ...
    constructor(world: World) {
        ...
        if (!TileLava.sharedAnimatorTimer) {
            TileLava.sharedAnimatorTimer = new SpritesheetAnimatorTimer();
        }

        this.name = 'LAVA';
        this.worldObjectType = WorldObjectType.Lava;

        const tileset = world.getTileset();

        const idleSpritesheet = tileset.getResourceByPath('Environment/Lava/Idle').texture;
        const animationSpritesheet = tileset.getResourceByPath('Environment/Lava/Animation').texture;
    ...
    public onCollisionStart(another: WorldObject, pair): void {
        if (another instanceof EntityPlayer || another instanceof EntityMonster) {
            another.die();
        }
    }
}