...
import WidgetDropdownItem from './WigetDropdownItem';
import Tapotan from '../../core/Tapotan';

type WidgetDropdownOption = {
    id: any;
    label: string;
}

export default class WidgetDropdown extends PIXI.Container {
    ...
    private currentItem: WidgetDropdownItem;

    private enabled: boolean = true;

    constructor(options: WidgetDropdownOption[], selectedOptionId: any = null) {
        super();
        ...
        options.forEach((option, idx) => {
            let item = new WidgetDropdownItem(option.label);
            item.name = option.id;
            this.addItem(item);
        });
        ...
        this.itemsContainerBackground.scale.set(
            Math.max(
                ...this.itemsContainer.children.map(x => (x as WidgetDropdownItem).width + (x.position.x * 2)),
...

        {
            ...
            this.handleItemClick(this.itemsContainer.children[selectedItemIndex] as WidgetDropdownItem);
        }
    }
    ...
    private handleApplicationStageClick = e => {
        if (!(e.target instanceof WidgetDropdownItem)) {
            this.hide();
        }
    }
    ...
    private addItem(item: WidgetDropdownItem) {
        item.on('click', () => {
            this.handleItemClick(item);
        });

        if (this.itemsContainer.children.length > 0) {
            let lastChild = this.itemsContainer.children[this.itemsContainer.children.length - 1] as WidgetDropdownItem;
    ...
    private handleItemClick(item: WidgetDropdownItem) {
        if (item === null) {
            return;
        }
